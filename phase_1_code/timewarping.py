...
# save interesting quantities for histograms
corr_x,corr_y = [],[]
Ediff_x,Ediff_y = [],[]
eccentrity = []
...
for fname in dirs[:2]:
    ...
    # now want to get N_new evenly-spaced points along the curve
    ...
    # compare the x and y coordinates to 1 period (with N_new = 250 points)
    # of an ideal sinusoid
    n = np.arange(len(x_eqdist))
    w_true = (2*pi)/float(N_new)
    x_true,y_true = np.cos(w_true*n),np.sin(w_true*n)
    # use maximum correlation to determine phase of x or y to determine amplitude
    ...
    # declare subplots
    xt,yt,xy,rtheta = fig_xt.add_subplot(211),fig_yt.add_subplot(211),fig_xy.add_subplot(111,aspect=1.0),fig_rtheta.add_subplot(111)
    xt_eqdist,yt_eqdist = fig_xt.add_subplot(212),fig_yt.add_subplot(212)
    # draw circles (actual and ideal)
    xy.plot(x_eqdist,y_eqdist,lw=2,label='$x_{eqdist}[n],y_{eqdist}[n]$')
